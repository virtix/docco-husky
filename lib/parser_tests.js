...
(function() {
  var assert, docco, dp, languages, parser, path, utils;
  ...
  suite("Spike", function() {
    return test('figure out for N of iterable', function() {
      var key, lang, _results;
      _results = [];
      for (key in languages) {
        lang = languages[key];
        _results.push(key + ' = ' + lang.name);
      }
      return _results;
    });
  });
  ...
  suite('Utils Tests', function() {
    test('Build a list of files from languages in the current working directory', function() {
      var ext, filter, lang_filter;
      filter = [];
      ...
      return lang_filter = lang_filter.join(' -o ');
    });
